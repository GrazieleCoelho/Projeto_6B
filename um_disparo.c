...
int64_t turn_off_green(alarm_id_t id, void *user_data) {
    ...
    printf("LED Verde desligado. Os LEDs AZUL e VERMELHO permanecem ligados.\n");
...
int64_t turn_off_blue(alarm_id_t id, void *user_data) {
    gpio_put(LED_BLUE, 0);
    printf("LED Azul desligado. LED VERMELHO permanece ligado.\n");
...
int64_t turn_off_red(alarm_id_t id, void *user_data) {
    gpio_put(LED_RED, 0);
    printf("LED VERMELHO desligado. FIM da sequência.\n");
...
void button_callback(uint gpio, uint32_t events) {
    ...
    if (!sequence_active) {
        ...
        printf("Botão pressionado! Iniciando o sistema de LEDs...\n");
...
int main() {
    ...
    printf("Sistema de LEDs iniciado.\n");
